<x-layout title="Approval Surat Cuti">
    ...
    <section class="bg-white  lg:w-3/4 sm:w-full px-4 py-6 sm:px-6 mx-auto shadow-lg rounded-lg">
        ...
        <div class="bg-white  p-4 sm:p-6 rounded-lg shadow-md transition-colors duration-300 ease-in-out">
            ...
            <div class="mb-4">
                ...
                <p class="text-md sm:text-lg text-gray-600 ">{{ $cuti->user->name }}</p>
            </div>
            <div class="mb-4">
                <p class="text-md sm:text-lg font-semibold text-gray-800 mb-1">Department:</p>
                <p class="text-md sm:text-lg text-gray-600 ">{{ $cuti->user->department->name }}</p>
            </div>
            <div class="mb-4">
                <p class="text-md sm:text-lg font-semibold text-gray-800 mb-1">Keterangan:</p>
                <p class="text-md sm:text-lg text-gray-600 ">{{ $cuti->keterangan }}</p>
            </div>
            <div class="mb-4">
                <p class="text-md sm:text-lg font-semibold text-gray-800 mb-1">Diajukan Pada:</p>
                <p class="text-md sm:text-lg text-gray-600 ">{{ \Carbon\Carbon::parse($cuti->created_at)->format('d F Y, H:i') }}</p>
            </div>
            <div class="mb-4">
                <p class="text-md sm:text-lg font-semibold text-gray-800 mb-1">Mulai Cuti:</p>
                <p class="text-md sm:text-lg text-gray-600 ">{{ \Carbon\Carbon::parse($cuti->start_date)->format('d F Y') }}</p>
            </div>
            <div class="mb-4">
                <p class="text-md sm:text-lg font-semibold text-gray-800 mb-1">Jumlah Hari Cuti:</p>
                <p class="text-md sm:text-lg text-gray-600 ">{{ $cuti->jumlah_hari }} Hari</p>
            </div>
            <div class="mb-4">
            </div>
            <!-- Action Buttons -->
            <div class="flex flex-col sm:flex-row justify-center space-y-2 sm:space-y-0 sm:space-x-2">
                ...
                <a href="{{route('cuti.print',$cuti->id)}}" target="_blank" rel="noopener noreferrer" class="flex-1 px-6 py-3 font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-300  transition-colors duration-300 text-center">
                    Detail
                </a>
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow-lg  transition-transform transform duration-300 scale-100">
                    ...
                    <div class="p-6 text-center">
                        ...
                        <form method="POST" action="{{route('cuti.approve',$cuti->id)}}" id="modalApproveForm" class="mt-4">
                            @csrf
                            @method('PUT')
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow ">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <form id="revisiForm" action="{{route('cuti.reject', $cuti->id)}}" method="POST">
                            @csrf
                            @method('PUT')
