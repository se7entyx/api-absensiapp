<x-layout title="Master Department">
    ...
    <section class="bg-gray-100 w-full relative px-4 py-4 sm:px-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
            ...
            <form class="w-full sm:w-2/3 max-w-lg sm:mr-4 m-0 p-0">
                ...
                <div class="relative">
                    ...
                    <input name="search" type="search" id="default-search" value="{{ old('search') }}" class="block w-full p-4 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 " placeholder="Search Department Name" autocomplete="off" />
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-semibold text-gray-600">Manager</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-6 py-4 text-sm text-gray-700">{{ $departments->firstItem() + $key}}</td>
                        <td class="px-6 py-4 text-sm text-gray-700">{{ $department->name}}</td>
                        <td class="px-6 py-4 text-sm text-gray-700">{{$department->leader->name ?? '-'}}</td>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-700 space-x-2">
                            <button type="button" id="edit-button" data-modal-target="editModal" data-modal-toggle="editModal" data-user-id="{{ $department->id }}" data-user-name="{{ $department->name }}" data-user-leader="{{ $department->user_id }}"  data-user-status="{{ $department->status }}" class="text-indigo-600 hover:underline">Edit</button>
        ...
        <div id="createModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div class="bg-white rounded-lg w-full max-w-md p-6 space-y-4">
                <h2 class="text-xl font-bold">Add Department</h2>
                <form method="post" action="{{route('department.store')}}">
                    ...
                    <div class="space-y-2">
                        ...
                        <div>
                            ...
                            <select class="w-full border rounded px-3 py-2" name="manager">
        ...
        <div id="editModal" class="fixed hidden inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div class="bg-white p-6 rounded shadow w-full max-w-md">
                ...
                <form id="editForm" method="post"action="{{ route('department.update', ':id') }}">
                    @csrf
                    ...
                    <div class="space-y-2">
                        ...
                        <div>
                            <label class="block text-sm">Manager</label>
                            <select class="w-full border rounded px-3 py-2" name="manager" id="editDepartment">
                                <option value="">Select</option>
                                @foreach ($users as $user)
                                <option value="{{$user['id']}}">{{$user['name']}}</option>
                                @endforeach
        ...
        document.addEventListener('DOMContentLoaded', () => {
            document.querySelectorAll('[data-user-id]').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    if (updateForm) {
                        const url = "{{ route('department.update', ':id') }}".replace(':id', id);
