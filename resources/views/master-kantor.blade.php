    ...
    <section class="bg-gray-100 w-full relative px-4 py-4 sm:px-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
            ...
            <form class="w-full sm:w-2/3 max-w-lg sm:mr-4 m-0 p-0">
                ...
                <div class="relative">
                    ...
                    <input name="search" type="search" id="default-search" value="{{ old('search') }}" class="block w-full p-4 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 " placeholder="Search Kantor Name" autocomplete="off" />
        ...
        <div class="overflow-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach ($kantors as $key => $kantor)
                    <tr>
                        <td class="px-6 py-4 text-sm text-gray-700">{{ $kantors->firstItem() + $key}}</td>
        ...
        <div id="editModal" class="fixed hidden inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div class="bg-white p-6 rounded shadow w-full max-w-md">
                <h2 class="text-lg font-bold mb-4">Edit Kantor</h2>
                <form id="editForm" method="post" action="{{ route('kantor.update', ':id') }}">
                    @csrf
                    @method('PUT')
                    <div class="space-y-2">
                        <div>
                        ...
                        <div>
                            <label class="block text-sm">Latitude</label>
                            <input type="text" id="editLat" class="w-full border rounded px-3 py-2" name="lat">
                        </div>
                        <div>
                            <label class="block text-sm">Logitude</label>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            document.querySelectorAll('[data-user-id]').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    if (updateForm) {
                        const url = "{{ route('kantor.update', ':id') }}".replace(':id', id);
